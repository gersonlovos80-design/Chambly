...
interface ClientReviewsScreenProps {
  onBack: () => void;
  clientName: string;
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5]">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {/* Title */}
        <div className="mb-6">
          <h1 className="text-3xl mb-2 text-[#1D1D1B]">Reseñas de {clientName}</h1>
          <p className="text-gray-600">{totalReviews} reseñas totales de profesionales</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-8">
          <h2 className="text-2xl mb-6 text-[#1D1D1B]">Comentarios de Profesionales</h2>
              ...
              {reviews.map((review) => (
                <div key={review.id} className="border-b last:border-b-0 pb-6 last:pb-0">
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h3 className="font-medium text-[#1D1D1B]">{review.professionalName}</h3>
